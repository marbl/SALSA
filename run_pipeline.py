...
def main():
    ...
    if not os.path.isfile(args.output+'/contig_links_iteration_'+str(iter_num)):
        try:
            cmd = 'python '+bin+'/make_links.py -b '+ args.bed + ' -d '+ args.output +' -i '+str(iter_num) + ' -x ' + args.dup
            print cmd
            log.write(cmd+'\n')
            p = subprocess.check_output(cmd,shell=True)
    ...
    while True:
        ...
        with open(args.output+'/scaffold_length_iteration_'+str(iter_num+1),'r') as f:
            ...
            if ng50[curr_sz - 1] == ng50[curr_sz - 2]:
                cmd ='python '+bin+'/get_seq.py -a '+ args.assembly + ' -f ' + args.output+'/scaffolds_FINAL.fasta -g ' + args.output+'/scaffolds_FINAL.agp -p '+args.output+'/scaffolds_iteration_'+str(iter_num-1)+'.p'
                log.write(cmd+'\n')
                os.system(cmd)
                sys.exit(1)
